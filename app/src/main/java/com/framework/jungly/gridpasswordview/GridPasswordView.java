package com.framework.jungly.gridpasswordview;
...
import com.framework.jungly.gridpasswordview.imebugfixer.ImeDelBugFixedEditText;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private com.framework.jungly.gridpasswordview.imebugfixer.ImeDelBugFixedEditText mInputView;
    private OnPasswordChangedListener mListener;
    private PasswordTransformationMethod mTransformationMethod;

    //按下是否显示软键盘
    private boolean mPressIsShowKeyboard;
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (textSize != -1) {
            this.mTextSize = SizeUtils.px2sp(context, textSize);
        }

        mLineWidth = (int) ta.getDimension(R.styleable.gridPasswordView_gpvLineWidth, SizeUtils.dp2px(getContext(), 1));
        ...
        //自己添加自定义属性
        mPressIsShowKeyboard = ta.getBoolean(R.styleable.gridPasswordView_gpvPressIsShowKeyboard, true);
    ...
    private void inflaterViews(Context context) {
        ...
        mInputView.setFocusable(mPressIsShowKeyboard);
        mInputView.setFocusableInTouchMode(mPressIsShowKeyboard);
        if(mPressIsShowKeyboard){
